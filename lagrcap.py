# Lagrangian, RELAXING CAPACITIES
import numpy as np
# -*- coding: utf-8 -*-
from pulp import *

def isFeasible(x, req, cap, cost, m, n):
   z = 0
   for i in range(m):
      capused = 0
      for j in range(n):
         capused += req[i][j]*x[i * n + j].varValue
...
def initialize_subpr(m,n,cost,cap,req,lambdas,categ):
   ...
   for j in range(n):
      ...
      for i in range(m):
         coeff[i*n+j]=1
      sumreq = pulp.lpSum([x_vars[i][j] for i in range(m)])
...
def run_subgr(cost, req, cap, max_iterations=50, categ="Integer",zub = float('inf')):
   ...
   while iteration <= max_iterations:
      ...
      prob2.setObjective(lpSum([(cost[i][j] - lambdas[i]) * x_vars[i][j] for i in range(m) for j in range(n)]))
      ...
      for i in range(m):
         for j in range(n):
            ...
            if prob2.variables()[i * n + j].varValue > 0: # if in solution
               subgrads[i] -= req[i][j] # (rhs) - sum req_ij*x_i
      for i in range(m):
         subgrads[i] += cap[i]      # rhs
         if(subgrads[i] < 0): subgrads[i] = 0
      sumSubGrad2 = sum(subgrads[i] * subgrads[i] for i in range(m))

      zubiter = isFeasible(prob2.variables(), req, cap, cost, m, n)
      if (zubiter >= 0):
         if (zubiter < zub):
            zub = zubiter
      else:
      ...
      if (zub - zlb < 1):
         ...
         lambdas = [(lambdas[i] + step * subgrads[i]) for i in range(m)]
         if (iteration % 10 == 0):
